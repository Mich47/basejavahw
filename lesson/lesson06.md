...
- Рефакторинг:
  ...
  - в конструктор `com.mich.webapp.model.Resume` добавьте второй параметр — `fullName`
  - во всех реализациях `Storage` замените метод `com.mich.webapp.model.Resume[] getAll()` на `List<com.mich.webapp.model.Resume> getAllSorted()`
